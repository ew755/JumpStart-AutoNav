...
const { writeHubLink, writeSiblingIdeStubs, ensureHubCursorRule } = require('./workspace-hub-link');
...
const { checkApproval } = require('../bin/lib/validator');
...
class WorkspaceManager {
  ...
  loadState() {
    try {
      if (!fs.existsSync(this.statePath)) {
        return { version: '1.0.0', project_locks: {} };
      }
  ...
  status() {
    ...
    this.config.projects.forEach((project) => {
      ...
      const external = isSiblingPath(project.path) ? '🔗 ' : '';
      console.log(`${icon} ${locked}${external}[${project.status}] ${projectId} — ${project.name}`);
      console.log(`     Path: ${project.path}${external ? ' (sibling repo)' : ''}`);
  ...
  validateDeps() {
    ...
    const errors = [];
    const blocked = [];
    const project_paths = validateAllProjectPaths(this.rootDir, this.config);
    const project_path_errors = [];

    project_paths.forEach((entry) => {
      entry.errors.forEach((message) => {
        project_path_errors.push(`[${entry.project_id}] ${message}`);
      });
    });
    ...
    return {
      valid: errors.length === 0 && project_path_errors.length === 0,
      errors: [...errors, ...project_path_errors],
      dependencies: deps,
      blocked,
      blocked_count: blocked.length,
      project_paths,
      sibling_count: project_paths.filter((p) => p.external).length,
      project_path_errors,
    };
  }

  validateProjectPaths() {
    return validateAllProjectPaths(this.rootDir, this.config);
  }
  ...
  isProjectLocked(projectId) {
    const lock = this.state.project_locks?.[projectId];
    return lock?.locked_by != null && !parallel.isLockExpired(lock);
  }
...
${(this.state.workspace_resume_context?.cross_project_dependencies || []).length === 0
  ...
  : this.state.workspace_resume_context.cross_project_dependencies
      .map((dep) => {
        const blocked = dep.blocked ? ' **BLOCKED**' : '';
        const unblock = dep.unblock_condition ? ` (unblock: ${dep.unblock_condition})` : '';
        return `- ${dep.from} → ${dep.to} (${dep.type})${blocked}${unblock}`;
      }).join('\n')}
      `);
      return;
    }
    ...
    if (format === 'json') {
      console.log(JSON.stringify({
        ...
        active_project: this.config.active_project,
  ...
  /**
   * Update workspace-level settings in projects.json.
   * @param {object} updates subset of settings keys
   * @returns {{ success: boolean, settings?: object, error?: string }}
   */
  configureSettings(updates = {}) {
    const booleanKeys = [
      'enforce_sequential_phases',
      'allow_parallel_projects',
      'pit_crew_review_required',
      'cross_project_dependency_validation',
      'aggregate_cost_tracking',
    ];
    const integerKeys = ['max_concurrent_projects'];
    const allowed = new Set([...booleanKeys, ...integerKeys]);

    const unknown = Object.keys(updates).filter((key) => !allowed.has(key));
    if (unknown.length > 0) {
      return { success: false, error: `Unknown setting(s): ${unknown.join(', ')}` };
    }
  ...
  validateSync(projectId = null) {
    ...
    projects.forEach((project) => {
      ...
      const projectPath = resolveProjectRoot(this.rootDir, project, projId);
      ...
      if (actualPhase >= 0) {
        ...
        phaseArtifacts.forEach((artifact) => {
          ...
          if (fs.existsSync(specPath)) {
            const { approved } = checkApproval(specPath);

            if (!approved && actualState.status === `phase-${actualPhase}`) {
              drifts.push({
                project_id: projId,
  ...
  /**
   * @param {{ id: string, name: string, type: string, approver?: string }} args
   */
  createProject(args = {}) {
  ...
  /**
   * Register an existing sibling repository (path outside hub, e.g. ../frontend).
   * @param {{ id: string, name: string, path: string, type?: string, approver?: string, init?: boolean, setActive?: boolean }} args
   */
  linkSiblingProject(args = {}) {
    if (!args.id || !args.name || !args.path) {
      return {
        success: false,
        error: 'Missing required arguments: --id, --name, --path (e.g. --path=../frontend)',
      };
    }

    if (!/^proj-[a-z0-9-]+$/.test(args.id)) {
      return { success: false, error: 'Invalid project ID. Must match: ^proj-[a-z0-9-]+$' };
    }

    if (this.config.projects.find((project) => this.getProjectId(project) === args.id)) {
      return { success: false, error: `Project already exists: ${args.id}` };
    }

    const absTarget = path.resolve(this.rootDir, args.path);
    if (!fs.existsSync(absTarget)) {
      return {
        success: false,
        error: `Sibling path not found: ${absTarget}. Clone the repo first or use --init after creating the directory.`,
      };
    }

    const hubResolved = path.resolve(this.rootDir);
    const targetResolved = path.resolve(absTarget);
    const isInsideHub = targetResolved === hubResolved || targetResolved.startsWith(hubResolved + path.sep);
    const relPath = toPosixRelative(this.rootDir, absTarget);

    if (isInsideHub && !relPath.startsWith('projects/') && relPath !== '.') {
      return {
        success: false,
        error: `Path is inside hub but not a standard projects/ layout. Use create-project for nested projects, or point --path at a sibling (../repo).`,
      };
    }

    if (args.init) {
      scaffoldMinimalProject(absTarget, {
        id: args.id,
        name: args.name,
        type: args.type || 'greenfield',
        approver: args.approver || 'TBD',
      });
    }

    const layout = validateProjectLayout(absTarget);
    if (!layout.valid) {
      return { success: false, error: layout.errors.join('; ') };
    }

    const configRel = `${relPath}/.jumpstart/config.yaml`.replace(/^\.\//, '');
    ...
    if (!this.config.active_project || args.setActive) {
      this.config.active_project = args.id;
    }

    this.saveConfig();

    writeHubLink(absTarget, {
      hubRoot: this.rootDir,
      projectId: args.id,
      projectName: args.name,
    });
    ensureHubCursorRule(this.rootDir);

    const siblingContext = getWorkspaceContext(absTarget);
    const ideFiles = writeSiblingIdeStubs(absTarget, siblingContext);
    ...
    return {
      ...
      external: isSiblingPath(relPath),
      warnings: layout.warnings,
      set_active: !!args.setActive || this.config.active_project === args.id,
      ide_files: ideFiles,
    };
  }
