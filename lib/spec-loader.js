...
const { inferOwnerProjectFromSpec, resolveProjectRoot } = require('./workspace-project-paths');
...
function loadSpec(context, specName, specDir = '') {
  ...
  if (context.mode === 'single-project') {
    specPath = path.join(rootDir, 'specs', specDir, specName);
...
/**
 * Infer which project owns a spec file (registry-aware for sibling repos).
 */
function inferOwnerProject(specPath, rootDir = process.cwd()) {
  return inferOwnerProjectFromSpec(specPath, rootDir);
}
