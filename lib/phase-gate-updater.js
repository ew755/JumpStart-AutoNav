...
function approvePhase(context, specPath, approverName) {
  try {
    ...
    let ownerProjectId;
    if (context.mode === 'multi-project' && context.project?.projectPath) {
      // Extract project ID from path like "projects/proj-sf-leads" or root "."
      const normalizedPath = context.project.projectPath.replace(/\\/g, '/');
      const pathMatch = normalizedPath.match(/projects\/([^/]+)/);
      ownerProjectId = pathMatch ? pathMatch[1] : (context.project.project_id || context.project.id || 'proj-default');
    } else {
