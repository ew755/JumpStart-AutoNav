...
function reconcileActiveProjectId(rootDir, workspaceState, projects) {
  const registryId = projects.active_project;
  const stateId = workspaceState.active_project_id;

  if (registryId && stateId && registryId !== stateId) {
    workspaceState.active_project_id = registryId;
    const statePath = path.join(rootDir, '.jumpstart', 'state', 'workspace-state.json');
    fs.writeFileSync(statePath, JSON.stringify(workspaceState, null, 2), 'utf8');
  }

  return workspaceState.active_project_id || registryId;
}
...
/**
 * Load active project metadata from workspace-state.json (hub cwd).
 */
function loadActiveProject(rootDir = process.cwd()) {
  try {
    ...
    const activeProjectId = reconcileActiveProjectId(rootDir, workspaceState, projects);
